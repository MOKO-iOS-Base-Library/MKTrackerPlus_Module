...
@interface MKTPScanPageCellTopView : UIView
...
@implementation MKTPScanPageCellTopView
...
@interface MKTPScanPageCellBottomView : UIView
...
@implementation MKTPScanPageCellBottomView
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKTrackerPlus", @"MKTPScanPageCell", @"tp_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
...
@interface MKTPScanPageCell ()

@property (nonatomic, strong)MKTPScanPageCellTopView *topView;

@property (nonatomic, strong)MKTPScanPageCellBottomView *bottomView;

@end

@implementation MKTPScanPageCell

+ (MKTPScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKTPScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKTPScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKTPScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKTPScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(tp_scanCellConnectButtonPressed:)]) {
        [self.delegate tp_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter

- (void)setDataModel:(MKTPTrackerModel *)dataModel {
    _dataModel = nil;
    ...
    if (dataModel.isTrackerPlus) {
        ...
        self.bottomView.batteryIcon.image = (dataModel.isCharging ? LOADICON(@"MKTrackerPlus", @"MKTPScanPageCell", @"tp_scan_batteryChargingIcon.png") : LOADICON(@"MKTrackerPlus", @"MKTPScanPageCell", @"tp_scan_batteryIcon.png"));
    }else {
        //V1设备
...
#pragma mark - getter
- (MKTPScanPageCellTopView *)topView {
    if (!_topView) {
        _topView = [[MKTPScanPageCellTopView alloc] init];
        [_topView.connectButton addTarget:self action:@selector(connectButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    }
    return _topView;
}

- (MKTPScanPageCellBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[MKTPScanPageCellBottomView alloc] init];
    }
    return _bottomView;
}
