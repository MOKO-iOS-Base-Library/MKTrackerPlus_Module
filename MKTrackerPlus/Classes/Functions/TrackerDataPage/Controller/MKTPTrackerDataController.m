...
@interface MKTPTrackerDataController ()<MFMailComposeViewControllerDelegate, UITableViewDelegate, UITableViewDataSource>
...
@implementation MKTPTrackerDataController

- (void)dealloc {
    NSLog(@"MKTPTrackerDataController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:mk_tp_receiveScannerTrackedDataNotification
                                                  object:nil];
}
...
- (void)leftButtonMethod {
    ...
    TICK;
    [[MKTPCentralManager shared] notifyScannerTrackedData:NO];
    if (self.parseTimer) {
    ...
    TOCK;
    [self performSelector:@selector(saveDataToLocal) withObject:nil afterDelay:0.5f];
}

#pragma mark - UITableViewDelegate
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSDictionary *pageModel = self.dataList[indexPath.row];
    return [MKTPTrackerDataCell fetchCellHeight:pageModel];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKTPTrackerDataCell *cell = [MKTPTrackerDataCell initCellWithTableView:tableView];
...
//        [self.dataList removeAllObjects];
    ...
    [[MKTPCentralManager shared] notifyScannerTrackedData:self.syncButton.selected];
}
...
- (void)readStorageDataNumber {
    ...
    [MKTPInterface tp_readStorageDataNumberWithSucBlock:^(id  _Nonnull returnData) {
...
- (void)deleteRecordDatas {
    ...
    [MKTPInterface tp_clearAllDatasWithSucBlock:^{
        [[MKHudManager share] hide];
        [self.dataList removeAllObjects];
        [MKTPDatabaseManager clearDataTable];
        [self performSelector:@selector(readStorageDataNumber) withObject:nil afterDelay:1.f];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
    }];
...
- (void)readDataFromDatabase {
    [[MKHudManager share] showHUDWithTitle:@"Reading..." inView:self.view isPenetration:NO];
    [MKTPDatabaseManager readDataListWithSucBlock:^(NSArray<NSDictionary *> * _Nonnull dataList) {
        [[MKHudManager share] hide];
...
- (void)parseContentDatas {
    ...
    NSArray *tempList = [MKTPAdopter parseScannerTrackedData:firstContent];
...
- (void)saveDataToLocal {
    [MKTPDatabaseManager clearDataTable];
    [MKTPDatabaseManager insertDataList:self.dataList sucBlock:^{
        [[MKHudManager share] hide];
...
- (UIView *)topView {
    if (!_topView) {
        _topView = [[UIView alloc] initWithFrame:CGRectMake(15, 0, kViewWidth - 2 * 15, 50.f)];
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKTrackerPlus", @"MKTPTrackerDataController", @"tp_threeAxisAcceLoadingIcon.png");
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKTrackerPlus", @"MKTPTrackerDataController", @"tp_slot_export_deleteIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKTrackerPlus", @"MKTPTrackerDataController", @"tp_slot_export_enableIcon.png") forState:UIControlStateNormal];
