...
NSString *const mk_tp_additionalInformation = @"mk_tp_additionalInformation";
NSString *const mk_tp_dataInformation = @"mk_tp_dataInformation";
NSString *const mk_tp_dataStatusLev = @"mk_tp_dataStatusLev";

@interface MKTPOperation ()
...
/**
 线程ID
 */
@property (nonatomic, assign)mk_tp_taskOperationID operationID;
...
/**
 线程结束时候的回调
 */
@property (nonatomic, copy)void (^completeBlock) (NSError *error, mk_tp_taskOperationID operationID, id returnData);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKTPTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKTPTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        mk_tp_additionalInformation:(self.additionalInformation ?: @{}),
                                        mk_tp_dataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        mk_tp_dataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_tp_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_tp_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    NSString *numString = returnData[mk_tp_communicationDataNum];
